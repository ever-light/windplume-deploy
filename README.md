...
部署页打开时每分钟刷新一次容器运行状态和操作历史；部署、回滚及生命周期操作期间
仍会单独快速刷新任务进度，操作完成后立即重新读取运行状态。页面切到后台时暂停定时刷新。
...
空间，不持续采集或保存历史。数据目录与系统盘共用文件系统时合并为一张磁盘卡片，
使用独立挂载点时才分别显示系统盘和数据盘。Docker
...
管理程序启动或打开页面时查询最新 Release，结果在进程内缓存 7 天；点击“检查更新”
会跳过缓存并立即查询 GitHub。
